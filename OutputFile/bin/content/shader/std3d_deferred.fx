...
PS_OUT PS_Std3D_Deferred(VS_OUT _in)
{
	...
	output.vColor = g_vec4_0;

	float3 vViewNormal = _in.vViewNormal;

	if (g_btex_0)
	{
		output.vColor = g_tex_0.Sample(g_sam_0, _in.vUV * g_vUVMapScale.xy + g_vUVMapLocation.xy);
	}

	if (g_btex_1)
	{
		float3 vNormal = g_tex_1.Sample(g_sam_0, _in.vUV * g_vUVMapScale.xy + g_vUVMapLocation.xy).xyz;
