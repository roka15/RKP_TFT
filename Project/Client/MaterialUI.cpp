...
int MaterialUI::render_update()
{
    ...
    tTextureMappingData mappingData = {};
    mappingData = pMtrl->GetUVMappingData();
    mappingData.vRotation = (mappingData.vRotation / XM_PI) * 180.f;

    bool check = pMtrl->IsUVMapping();
    ImGui::Checkbox("Use UVMapping Setting", &check);
    pMtrl->SetUVMapping(check);
    ...
    if (check)
    {
        ...
        pMtrl->SetUVMappingData(mappingData);
    }
